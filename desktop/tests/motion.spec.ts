...
test("joint preview moves displayed geometry without rebuilding or editing the project", async () => {
  ...
  try {
    ...
    const angle = page.getByRole("slider", { name: "spin angle" });
    ...
    await page.getByRole("button", { name: "Play spin", exact: true }).click();
    await expect(
    ...
    await page.screenshot({ path: test.info().outputPath("motion-panel.png") });
