...
A desktop prototype for inspecting real CadKit projects. Electron hosts a
...
For a supplied trial bundle, follow its `docs/install.md` to install the Python
wheel and copy a writable desktop runtime. The commands below also apply to
source checkouts. A consumer with a `src/` package layout must be installed in
the selected worker environment or supplied through `PYTHONPATH`.
...
Node 22.12+ is required. The prototype uses the project's existing Python
environment; it is not yet a packaged installer with a bundled Python runtime.
...
`project:PROJECT`. The default Python is `.venv/bin/python` in the project
folder (`.venv/Scripts/python.exe` on Windows), falling back to `python3`.
...
into your MCP client's server settings. A generic configuration looks like:
...
native app dialogs and cannot be changed through MCP. Linux is integration
tested; Windows and macOS still need platform testing.
